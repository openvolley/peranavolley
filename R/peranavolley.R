#' \pkg{peranavolley}
#'
#' Functions for reading and working with Perana Sports volleyball scouting files.
#'
#' @name peranavolley
#' @author Ben Raymond \email{ben@@untan.gl}
#' @docType package
#' @importFrom assertthat assert_that is.flag is.string
#' @importFrom datavolley dv_xy2cone dv_xy2index home_team inspect teams visiting_team
#' @importFrom dplyr as_tibble bind_rows case_when distinct everything filter group_by lag lead left_join mutate rename row_number select slice tibble tribble ungroup %>%
#' @importFrom lubridate as.duration hms ymd_hms
#' @importFrom rlang .data
#' @importFrom stats na.omit setNames
#' @importFrom stringr str_match str_trim
#' @importFrom utils read.csv

NULL
